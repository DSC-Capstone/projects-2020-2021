...
def autophrase(filename,save_path,direct_path,raw_path):
    mycwd = os.getcwd()
    os.rename(raw_path+filename+'.txt', direct_path+filename+'.txt')
    os.chdir('AutoPhrase')
...
DEFAULT_TRAIN=${DATA_DIR}/EN/'''+filename+'''.txt
...
### END Generating Output for Checking Quality ###
    ...
    os.remove('phrasal_segmentation.sh')
    with open ('phrasal_segmentation.sh', 'w') as rsh:
        rsh.write('''\
#!/bin/bash
...
TEXT_TO_SEG=${TEXT_TO_SEG:- ${DATA_DIR}/EN/'''+filename+'''.txt}
...
### END Generating Output for Checking Quality ###
    ...
    subprocess.run(['./auto_phrase.sh',filename+'.txt'])
    subprocess.run(['chmod','+x','phrasal_segmentation.sh'])
    subprocess.run(['./phrasal_segmentation.sh'])

    os.chdir(mycwd)
    os.rename('./AutoPhrase/models/DBLP/AutoPhrase.txt', save_path+'AutoPhrase.txt')
    os.rename('./AutoPhrase/models/DBLP/AutoPhrase_multi-words.txt', save_path+'AutoPhrase_multi-words.txt')
    os.rename('./AutoPhrase/models/DBLP/AutoPhrase_single-word.txt', save_path+'AutoPhrase_single-word.txt')
    os.rename('./AutoPhrase/models/DBLP/token_mapping.txt', save_path+'token_mapping.txt')
    os.rename('./AutoPhrase/models/DBLP/segmentation.model', save_path+'segmentation.model')
    os.rename(direct_path+filename+'.txt',raw_path+filename+'.txt')
