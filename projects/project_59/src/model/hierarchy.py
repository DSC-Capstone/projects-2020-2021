...
def generate_hierarchy(
    ...
    **kwargs,
    ...
    print("==> Wrote tree to {}".format(path))
...
def test_hierarchy(dataset,path):
    wnids = get_wnids_from_dataset(dataset)
    ...
    if num_roots == 1:
        print("Found just 1 root.")
    else:
        print(f"Found {num_roots} roots. Should be only 1.")
...
def generate_vis(
    ...
    if verbose:
        print("==> Wrote HTML to {}".format(path_html))
...
def generate_hierarchy_vis(path, dataset):
    print("==> Reading from {}".format(path))
    G = read_graph(path)

    path_html = f"./data/out/graph-induced-densenet121-snakes.html"

    return generate_hierarchy_vis_from(
        G, dataset, path_html, verbose=True
    )
...
def generate_hierarchy_vis_from(
    ...
    **kwargs
    ...
    if num_roots > 1:
        print(f"Found {num_roots} roots! Should be only 1: {roots}")
    elif verbose:
