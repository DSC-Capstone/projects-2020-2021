...
def atanh(x, eps=1e-6):
    """
    Deprecated. Torch 1.7.1 now implements a stable atanh function.
...
class L2Adversary(object):
    ...
    def __call__(self, model, inputs, targets, to_numpy=True):
        ...
        :param inputs: the original images tensor, of dimension [B x n_features].
...
#             pert_tanh = torch.clamp(pert_tanh, min=1)
    ...
    def _optimize(self, model, optimizer, inputs_tanh_var, pert_tanh_var,
        ...
        :return: the batch loss, squared L2-norm of adversarial perturbations
                 ...
                 [B]), the adversarial examples (of dimension [B x n_features])
        """
